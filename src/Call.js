...
var Call = function(clientObj, callID, params, listeners) {
  ...
  if (audioVideoDOMContainerObj == null) {
    ...
    LOGGER.log(
      "Inserting HTML5 <video/> element (audio only) to user provided tag " +
        tagId
    );
    audioVideoDOMContainerObj.appendChild(this.remoteAudioVideo);
  }
...
function _attachJanusVideoPlugin(callbackSuccess) {
  ...
  this.janusInstance.attach({
    ...
    success: function(pluginHandle) {
      ...
      Janus.log(
        "Plugin attached! (" +
          self.janusVideoPlugin.getPlugin() +
          ", id=" +
          self.janusVideoPlugin.getId() +
          ")"
      );
    ...
    onmessage: function(msg, jsep) {
          ...
          if (msg["publishers"] !== undefined && msg["publishers"] !== null) {
            ...
            for (let i = 1; i < 6; i++) {
              if (
                ...
                self.janusFeeds[i].rfid == leaving
              ) {
            ...
            for (let i = 1; i < 6; i++) {
              if (
                self.janusFeeds[i] != null &&
                self.janusFeeds[i] != undefined &&
                self.janusFeeds[i].rfid == unpublished
              ) {
                remoteFeed = self.janusFeeds[i];
                break;
              }
            ...
            if (remoteFeed != null) {
              ...
              let target = document.querySelector(
                ...
                } div[remoteid='${remoteFeed.rfindex}']`
              );
              target.innerHTML = "";
      ...
      if (jsep !== undefined && jsep !== null) {
        ...
        if (
          ...
          self.janusVideoStream.getAudioTracks().length > 0 &&
          !audio
        ) {
        ...
        if (
          ...
          self.janusVideoStream.getVideoTracks().length > 0 &&
          !video
        ) {
    ...
    onlocalstream: function(stream) {
      ...
      if (
        ...
        typeof localVideoElement === "undefined"
      ) {
      ...
      if (
        self.janusVideoPlugin.webrtcStuff.pc.iceConnectionState !==
          "completed" &&
...
function initVideoInConferenceRoom(
  ...
  if (
    document.getElementById(this.janusVideoOptions.localVideoContainerId) ==
    null
  ) {
  ...
  if (
    document.getElementById(this.janusVideoOptions.remoteVideosContainerId) ==
    null
  ) {
    ...
    callback: function() {
      ...
      self.janusInstance = new Janus({
        ...
        destroyed: function() {
          ...
          const localVideoContainerElement = document.getElementById(
            self.janusVideoOptions.localVideoContainerId
          );
          ...
          const remoteVideosContainerElement = document.getElementById(
            self.janusVideoOptions.remoteVideosContainerId
          );
