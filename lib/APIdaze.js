...
var handleWebSocketMessage = function handleWebSocketMessage(event) {
  ...
  var callID = json.params && json.params.callID;
 ...
 */
var handleWebSocketClose = function handleWebSocketClose() {
...
var sendText = function sendText(_ref, userCallback) {
  ...
  request.params = {
    ...
    text: text,
    userKeys: (typeof userKeys === "undefined" ? "undefined" : _typeof(userKeys)) !== "object" ? {} : userKeys
  };
...
var Call = function Call(clientObj, callID, params, listeners) {
  ...
  if (audioVideoDOMContainerObj == null) {
    LOGGER.log("Inserting HTML5 <video/> element " + (this.activateVideo ? "(video on) " : "(video off) ") + "to APIdaze tag " + tagId);
    ...
    LOGGER.log("Inserting HTML5 <video/> element " + (this.activateVideo ? "(video on) " : "(video off) ") + "to user provided tag " + tagId);
    audioVideoDOMContainerObj.appendChild(this.remoteAudioVideo);
  }
...
function handleRoomChatMessage(message) {
  ...
  typeof this.userRoomChatMessageCallback === "function" && this.userRoomChatMessageCallback(message);
}
...
module.exports = function(dependencies, opts) {
  ...
  var commonShim = __webpack_require__(17) || null;
